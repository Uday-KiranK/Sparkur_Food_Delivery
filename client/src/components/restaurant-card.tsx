...
const RestaurantCard = ({ restaurant, featured = false }: RestaurantCardProps) => {
  ...
  return (
    <Link href={`/restaurant/${id}`}>
      <div className={`${featured ? 'rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow border border-gray-100 cursor-pointer' : 'cursor-pointer group'}`}>
        {/* Image Container */}
        <div className={`${featured ? 'h-44' : 'h-52 rounded-lg overflow-hidden'} relative`}>
          <img 
            src={image_url} 
          ...
          <div className="absolute top-3 right-3">
            <div className={`${is_veg ? 'border-green-600 text-green-600' : 'border-red-500 text-red-500'} bg-white border w-6 h-6 flex items-center justify-center rounded-md shadow-sm`}>
              {is_veg ? 
        ...
        <div className={`${featured ? 'p-4' : 'pt-3'}`}>
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-lg text-[#3d4152] truncate">{name}</h3>
            
            {/* Rating Badge */}
            <div className="flex items-center gap-1">
              <span className={`${displayRating >= 4.0 ? 'bg-[#60b246]' : displayRating >= 3.5 ? 'bg-[#db7c38]' : 'bg-[#e1b055]'} text-white text-xs px-1.5 py-0.5 rounded flex items-center font-medium`}>
          ...
          {/* Restaurant Details */}
          <div className="mt-1">
            <p className="text-[#686b78] text-sm font-medium truncate">{cuisine_types.join(', ')}</p>
