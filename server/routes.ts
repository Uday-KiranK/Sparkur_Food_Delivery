  ...
  app.get("/api/restaurants", async (req, res) => {
    try {
      ...
      if (
        adminOnly &&
        req.isAuthenticated() &&
        req.user.role === UserRole.RESTAURANT_ADMIN
      ) {
  ...
  app.post("/api/restaurants", async (req, res) => {
    try {
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can create restaurants" });
      }
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid restaurant data", errors: error.format() });
      }
  ...
  app.put("/api/restaurants/:id", async (req, res) => {
    try {
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can update restaurants" });
      }
      ...
      if (restaurant.admin_id !== req.user.id) {
        return res
          .status(403)
          .json({ message: "You can only update your own restaurants" });
      }
  ...
  app.post("/api/restaurants/:id/menu", async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can add menu items" });
      }
      ...
      if (restaurant.admin_id !== req.user.id) {
        console.log(
          "Restaurant admin mismatch. Restaurant admin:",
          restaurant.admin_id,
          "User:",
          req.user.id,
        );
        return res
          .status(403)
          .json({ message: "You can only add items to your own restaurants" });
      }
      ...
      try {
        ...
        if (err instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid menu item data", errors: err.format() });
        }
        res
          .status(500)
          .json({ message: "Failed to create menu item", error: String(err) });
      }
    } catch (error) {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid menu item data", errors: error.format() });
      }
  ...
  app.put("/api/menu-items/:id", async (req, res) => {
    try {
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can update menu items" });
      }
  ...
  app.delete("/api/menu-items/:id", async (req, res) => {
    try {
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can delete menu items" });
      }
  ...
  app.post("/api/restaurants/:id/categories", async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Only restaurant admins can add categories" });
      }
      ...
      try {
        ...
        if (err instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid category data", errors: err.format() });
        }
        res
          .status(500)
          .json({ message: "Failed to create category", error: String(err) });
      }
    } catch (error) {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid category data", errors: error.format() });
      }
    ...
    try {
      ...
      if (
        ...
        if (!restaurant || restaurant.admin_id !== req.user.id) {
          return res
            .status(403)
            .json({ message: "You can only view orders for your restaurants" });
        }
      } else if (
        req.user.role === UserRole.DELIVERY_PARTNER &&
        ...
        return res
          ...
          .json({
            message:
              "You can only view orders assigned to you or ready for pickup",
          });
      }
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid order data", errors: error.format() });
      }
      ...
      if (req.user.role === UserRole.CUSTOMER) {
        ...
        if (
          ...
          return res
            ...
            .json({
              message:
                "Cannot cancel order that is already being delivered or delivered",
            });
        }
      } else if (req.user.role === UserRole.RESTAURANT_ADMIN) {
        const restaurant = await storage.getRestaurant(order.restaurant_id);
        if (!restaurant || restaurant.admin_id !== req.user.id) {
          return res
            .status(403)
            .json({
              message: "You can only update orders for your restaurants",
            });
        }
        ...
        if (
          ...
          return res
            ...
            .json({
              message:
                "Restaurant admins can only confirm, prepare, or mark orders as ready",
            });
        }
      } else if (req.user.role === UserRole.DELIVERY_PARTNER) {
        ...
        if (status === OrderStatus.OUT_FOR_DELIVERY) {
          if (order.status !== OrderStatus.READY_FOR_PICKUP) {
            return res
              .status(400)
              .json({ message: "Can only pick up orders that are ready" });
          }
          ...
          if (order.delivery_partner_id !== req.user.id) {
            return res
              .status(403)
              .json({
                message: "You can only mark your assigned orders as delivered",
              });
          }
          ...
          if (order.status !== OrderStatus.OUT_FOR_DELIVERY) {
            return res
              ...
              .json({
                message:
                  "Can only mark orders that are out for delivery as delivered",
              });
          }
        } else {
          return res
            ...
            .json({
              message:
                "Delivery partners can only update to out for delivery or delivered",
            });
        }
      }
